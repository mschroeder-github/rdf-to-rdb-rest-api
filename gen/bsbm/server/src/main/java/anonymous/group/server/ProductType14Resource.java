...
import anonymous.group.api.ProductType14;
...
public class ProductType14Resource {

    private DatabaseController dbc;

    public ProductType14Resource(DatabaseController dbc) {
        this.dbc = dbc;

        Spark.get("/productType14", this::getProductType14);
        Spark.get("/productType14/:id", this::getProductType14Id);

        Spark.delete("/productType14", this::deleteProductType14);
        Spark.delete("/productType14/:id", this::deleteProductType14Id);

        Spark.post("/productType14", this::postProductType14);

        Spark.put("/productType14/:id", (req, resp) -> putOrPatchProductType14Id(req, resp, true));
        Spark.patch("/productType14/:id", (req, resp) -> putOrPatchProductType14Id(req, resp, false));

        Spark.options("/productType14", this::optionsProductType14);
    }

    public Object getProductType14(Request req, Response resp) {
        return getProductType14(req, resp, null);
    } 

    public Object getProductType14Id(Request req, Response resp) {
        return getProductType14(req, resp, getIds(req));
    }

    private Object getProductType14(Request req, Response resp, List<Long> ids) {
        ...
        result.put("type", "ProductType14");
        
        List<ProductType14> records = dbc.selectProductType14(ids, offset, limit, rqlNode);
    ...
    public Object postProductType14(Request req, Response resp) {
        JSONObject json = toJSONObject(req);
        ProductType14 record = fromJSONObject(json);
        dbc.insertProductType14(Arrays.asList(record));
        resp.header("Location", "/productType14/" + record.getId());
        resp.status(201);
        return "";
    }

    public Object putOrPatchProductType14Id(Request req, Response resp, boolean put) {
        List<Long> ids = getIds(req);
        JSONObject json = toJSONObject(req);
        ProductType14 record = fromJSONObject(json);
        record.setId(ids.get(0));
        List<ProductType14> records = dbc.selectProductType14(ids, null, null, null);
        if(records.isEmpty()) {
            resp.status(404);
            return "no ProductType14 with id " + ids.get(0);
        }
        dbc.updateProductType14(Arrays.asList(record), put);
        resp.status(204);
        return "";
    }

    public Object deleteProductType14(Request req, Response resp) {
        dbc.deleteProductType14();
        resp.status(204);
        return "";
    }

    public Object deleteProductType14Id(Request req, Response resp) {
        dbc.deleteProductType14(getIds(req));
        resp.status(204);
        return "";
    }

    public Object optionsProductType14(Request req, Response resp) {
    ...
    public JSONArray toJSONArray(List<ProductType14> records) {
    ...
    public JSONObject toJSONObject(ProductType14 record) {
        JSONObject result = new JSONObject();
        JSONUtils.forceLinkedHashMap(result);
    ...
    public ProductType14 fromJSONObject(JSONObject jsonObject) {
