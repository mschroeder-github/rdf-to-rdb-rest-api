...
import anonymous.group.api.ProductType16;
...
public class ProductType16Resource {

    private DatabaseController dbc;

    public ProductType16Resource(DatabaseController dbc) {
        this.dbc = dbc;

        Spark.get("/productType16", this::getProductType16);
        Spark.get("/productType16/:id", this::getProductType16Id);

        Spark.delete("/productType16", this::deleteProductType16);
        Spark.delete("/productType16/:id", this::deleteProductType16Id);

        Spark.post("/productType16", this::postProductType16);

        Spark.put("/productType16/:id", (req, resp) -> putOrPatchProductType16Id(req, resp, true));
        Spark.patch("/productType16/:id", (req, resp) -> putOrPatchProductType16Id(req, resp, false));

        Spark.options("/productType16", this::optionsProductType16);
    }

    public Object getProductType16(Request req, Response resp) {
        return getProductType16(req, resp, null);
    } 

    public Object getProductType16Id(Request req, Response resp) {
        return getProductType16(req, resp, getIds(req));
    }

    private Object getProductType16(Request req, Response resp, List<Long> ids) {
        ...
        result.put("type", "ProductType16");
        
        List<ProductType16> records = dbc.selectProductType16(ids, offset, limit, rqlNode);
    ...
    public Object postProductType16(Request req, Response resp) {
        JSONObject json = toJSONObject(req);
        ProductType16 record = fromJSONObject(json);
        dbc.insertProductType16(Arrays.asList(record));
        resp.header("Location", "/productType16/" + record.getId());
        resp.status(201);
        return "";
    }

    public Object putOrPatchProductType16Id(Request req, Response resp, boolean put) {
        List<Long> ids = getIds(req);
        JSONObject json = toJSONObject(req);
        ProductType16 record = fromJSONObject(json);
        record.setId(ids.get(0));
        List<ProductType16> records = dbc.selectProductType16(ids, null, null, null);
        if(records.isEmpty()) {
            resp.status(404);
            return "no ProductType16 with id " + ids.get(0);
        }
        dbc.updateProductType16(Arrays.asList(record), put);
        resp.status(204);
        return "";
    }

    public Object deleteProductType16(Request req, Response resp) {
        dbc.deleteProductType16();
        resp.status(204);
        return "";
    }

    public Object deleteProductType16Id(Request req, Response resp) {
        dbc.deleteProductType16(getIds(req));
        resp.status(204);
        return "";
    }

    public Object optionsProductType16(Request req, Response resp) {
    ...
    public JSONArray toJSONArray(List<ProductType16> records) {
    ...
    public JSONObject toJSONObject(ProductType16 record) {
        JSONObject result = new JSONObject();
        JSONUtils.forceLinkedHashMap(result);
    ...
    public ProductType16 fromJSONObject(JSONObject jsonObject) {
