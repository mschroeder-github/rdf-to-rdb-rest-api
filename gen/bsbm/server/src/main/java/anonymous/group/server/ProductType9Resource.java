...
import anonymous.group.api.ProductType9;
...
public class ProductType9Resource {

    private DatabaseController dbc;

    public ProductType9Resource(DatabaseController dbc) {
        this.dbc = dbc;

        Spark.get("/productType9", this::getProductType9);
        Spark.get("/productType9/:id", this::getProductType9Id);

        Spark.delete("/productType9", this::deleteProductType9);
        Spark.delete("/productType9/:id", this::deleteProductType9Id);

        Spark.post("/productType9", this::postProductType9);

        Spark.put("/productType9/:id", (req, resp) -> putOrPatchProductType9Id(req, resp, true));
        Spark.patch("/productType9/:id", (req, resp) -> putOrPatchProductType9Id(req, resp, false));

        Spark.options("/productType9", this::optionsProductType9);
    }

    public Object getProductType9(Request req, Response resp) {
        return getProductType9(req, resp, null);
    } 

    public Object getProductType9Id(Request req, Response resp) {
        return getProductType9(req, resp, getIds(req));
    }

    private Object getProductType9(Request req, Response resp, List<Long> ids) {
        ...
        result.put("type", "ProductType9");
        
        List<ProductType9> records = dbc.selectProductType9(ids, offset, limit, rqlNode);
    ...
    public Object postProductType9(Request req, Response resp) {
        JSONObject json = toJSONObject(req);
        ProductType9 record = fromJSONObject(json);
        dbc.insertProductType9(Arrays.asList(record));
        resp.header("Location", "/productType9/" + record.getId());
        resp.status(201);
        return "";
    }

    public Object putOrPatchProductType9Id(Request req, Response resp, boolean put) {
        List<Long> ids = getIds(req);
        JSONObject json = toJSONObject(req);
        ProductType9 record = fromJSONObject(json);
        record.setId(ids.get(0));
        List<ProductType9> records = dbc.selectProductType9(ids, null, null, null);
        if(records.isEmpty()) {
            resp.status(404);
            return "no ProductType9 with id " + ids.get(0);
        }
        dbc.updateProductType9(Arrays.asList(record), put);
        resp.status(204);
        return "";
    }

    public Object deleteProductType9(Request req, Response resp) {
        dbc.deleteProductType9();
        resp.status(204);
        return "";
    }

    public Object deleteProductType9Id(Request req, Response resp) {
        dbc.deleteProductType9(getIds(req));
        resp.status(204);
        return "";
    }

    public Object optionsProductType9(Request req, Response resp) {
    ...
    public JSONArray toJSONArray(List<ProductType9> records) {
    ...
    public JSONObject toJSONObject(ProductType9 record) {
        JSONObject result = new JSONObject();
        JSONUtils.forceLinkedHashMap(result);
    ...
    public ProductType9 fromJSONObject(JSONObject jsonObject) {
