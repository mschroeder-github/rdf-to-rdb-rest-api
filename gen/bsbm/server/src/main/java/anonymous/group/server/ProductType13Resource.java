...
import anonymous.group.api.ProductType13;
...
public class ProductType13Resource {

    private DatabaseController dbc;

    public ProductType13Resource(DatabaseController dbc) {
        this.dbc = dbc;

        Spark.get("/productType13", this::getProductType13);
        Spark.get("/productType13/:id", this::getProductType13Id);

        Spark.delete("/productType13", this::deleteProductType13);
        Spark.delete("/productType13/:id", this::deleteProductType13Id);

        Spark.post("/productType13", this::postProductType13);

        Spark.put("/productType13/:id", (req, resp) -> putOrPatchProductType13Id(req, resp, true));
        Spark.patch("/productType13/:id", (req, resp) -> putOrPatchProductType13Id(req, resp, false));

        Spark.options("/productType13", this::optionsProductType13);
    }

    public Object getProductType13(Request req, Response resp) {
        return getProductType13(req, resp, null);
    } 

    public Object getProductType13Id(Request req, Response resp) {
        return getProductType13(req, resp, getIds(req));
    }

    private Object getProductType13(Request req, Response resp, List<Long> ids) {
        ...
        result.put("type", "ProductType13");
        
        List<ProductType13> records = dbc.selectProductType13(ids, offset, limit, rqlNode);
    ...
    public Object postProductType13(Request req, Response resp) {
        JSONObject json = toJSONObject(req);
        ProductType13 record = fromJSONObject(json);
        dbc.insertProductType13(Arrays.asList(record));
        resp.header("Location", "/productType13/" + record.getId());
        resp.status(201);
        return "";
    }

    public Object putOrPatchProductType13Id(Request req, Response resp, boolean put) {
        List<Long> ids = getIds(req);
        JSONObject json = toJSONObject(req);
        ProductType13 record = fromJSONObject(json);
        record.setId(ids.get(0));
        List<ProductType13> records = dbc.selectProductType13(ids, null, null, null);
        if(records.isEmpty()) {
            resp.status(404);
            return "no ProductType13 with id " + ids.get(0);
        }
        dbc.updateProductType13(Arrays.asList(record), put);
        resp.status(204);
        return "";
    }

    public Object deleteProductType13(Request req, Response resp) {
        dbc.deleteProductType13();
        resp.status(204);
        return "";
    }

    public Object deleteProductType13Id(Request req, Response resp) {
        dbc.deleteProductType13(getIds(req));
        resp.status(204);
        return "";
    }

    public Object optionsProductType13(Request req, Response resp) {
    ...
    public JSONArray toJSONArray(List<ProductType13> records) {
    ...
    public JSONObject toJSONObject(ProductType13 record) {
        JSONObject result = new JSONObject();
        JSONUtils.forceLinkedHashMap(result);
    ...
    public ProductType13 fromJSONObject(JSONObject jsonObject) {
