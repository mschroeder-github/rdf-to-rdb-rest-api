...
import anonymous.group.api.ProductType18;
...
public class ProductType18Resource {

    private DatabaseController dbc;

    public ProductType18Resource(DatabaseController dbc) {
        this.dbc = dbc;

        Spark.get("/productType18", this::getProductType18);
        Spark.get("/productType18/:id", this::getProductType18Id);

        Spark.delete("/productType18", this::deleteProductType18);
        Spark.delete("/productType18/:id", this::deleteProductType18Id);

        Spark.post("/productType18", this::postProductType18);

        Spark.put("/productType18/:id", (req, resp) -> putOrPatchProductType18Id(req, resp, true));
        Spark.patch("/productType18/:id", (req, resp) -> putOrPatchProductType18Id(req, resp, false));

        Spark.options("/productType18", this::optionsProductType18);
    }

    public Object getProductType18(Request req, Response resp) {
        return getProductType18(req, resp, null);
    } 

    public Object getProductType18Id(Request req, Response resp) {
        return getProductType18(req, resp, getIds(req));
    }

    private Object getProductType18(Request req, Response resp, List<Long> ids) {
        ...
        result.put("type", "ProductType18");
        
        List<ProductType18> records = dbc.selectProductType18(ids, offset, limit, rqlNode);
    ...
    public Object postProductType18(Request req, Response resp) {
        JSONObject json = toJSONObject(req);
        ProductType18 record = fromJSONObject(json);
        dbc.insertProductType18(Arrays.asList(record));
        resp.header("Location", "/productType18/" + record.getId());
        resp.status(201);
        return "";
    }

    public Object putOrPatchProductType18Id(Request req, Response resp, boolean put) {
        List<Long> ids = getIds(req);
        JSONObject json = toJSONObject(req);
        ProductType18 record = fromJSONObject(json);
        record.setId(ids.get(0));
        List<ProductType18> records = dbc.selectProductType18(ids, null, null, null);
        if(records.isEmpty()) {
            resp.status(404);
            return "no ProductType18 with id " + ids.get(0);
        }
        dbc.updateProductType18(Arrays.asList(record), put);
        resp.status(204);
        return "";
    }

    public Object deleteProductType18(Request req, Response resp) {
        dbc.deleteProductType18();
        resp.status(204);
        return "";
    }

    public Object deleteProductType18Id(Request req, Response resp) {
        dbc.deleteProductType18(getIds(req));
        resp.status(204);
        return "";
    }

    public Object optionsProductType18(Request req, Response resp) {
    ...
    public JSONArray toJSONArray(List<ProductType18> records) {
    ...
    public JSONObject toJSONObject(ProductType18 record) {
        JSONObject result = new JSONObject();
        JSONUtils.forceLinkedHashMap(result);
    ...
    public ProductType18 fromJSONObject(JSONObject jsonObject) {
